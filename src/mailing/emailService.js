...
function coverImageBlock(imageUrl, alt) {
  return imageUrl
    ? `<img src="${imageUrl}" alt="${alt}" width="160" height="160" style="border-radius: 12px; display:block; margin: 0 auto; object-fit: cover;" />`
  ...
  const bodyHtml = `
    <tr>
      <td style="padding: 24px 32px 8px 32px; text-align:center;">
        ...
        ${coverImageBlock(song.imageUrl, song.title)}
...
async function sendNewAlbumNotification(recipients, album) {
  const bodyHtml = `
    <tr>
      <td style="padding: 24px 32px 8px 32px; text-align:center;">
        <p style="margin: 0 0 16px 0; font-size: 12px; font-weight: 600; letter-spacing: 1px; text-transform: uppercase; color:${BRAND_COLOR};">
        ...
        ${coverImageBlock(album.imageUrl, album.title)}
        <h1 style="margin: 20px 0 4px 0; font-size: 20px; color:${TEXT_COLOR};">
          ${album.title}
        </h1>
        <p style="margin: 0; font-size: 14px; color:${MUTED_COLOR};">
  ...
  await transporter.sendMail({
    from: `"Muse" <${process.env.GMAIL_USER}>`,
    bcc: recipients,
    subject: `New album: ${album.title} by ${album.artist}`,
    html: baseLayout({ preheader: `${album.artist} just dropped a new album on Muse`, bodyHtml }),
  });
}

export default { sendWelcomeEmail, sendNewSongNotification, sendNewAlbumNotification };